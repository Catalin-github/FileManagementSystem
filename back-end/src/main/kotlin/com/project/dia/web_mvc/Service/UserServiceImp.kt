...
import com.project.dia.web_mvc.Model.Entity.*
...
import com.project.dia.web_mvc.repository.ImageRepository
...
import com.project.dia.web_mvc.repository.VideoRepository
...
class UserServiceImp : UserService {
    ...
    override fun saveUser(user: RegisterModel, response: HttpServletResponse): ModelResponse {
        val userExists = userDao.checkUserEmailExist(user.email)
        println("you are not in DB $userExists")
...
//            val token: String? = jwtUtils.generateToken(userEntity.email, claims)
//            jwtUtils.addCookieToResponse(token, response)
    ...
    override fun findAllImage(id: String): MutableList<FileModel>? {

        val images = imageRepository.findAllImageModelByUserEmail(id)
        return if (images.isNotEmpty()) {
            val imageResponse = mutableListOf<FileModel>()
            for (image in images) {
                imageResponse.add(FileModel(id = image.id, name = image.name, type = image.type, data = image.data))
            }
            imageResponse
        } else {
            mutableListOf()
    ...
    override fun uploadAllImage(email: String, files: MutableList<FileModel>): Boolean {
        val user = userDao.findUserByEmail(email)
        return run {
            for (image in files) {
                imageRepository.save(ImageEntity(name = image.name, type = image.type, data = image.data, user = user))
            }
            true
        }
    }
    ...
    override fun removeImage(id: Long): Boolean {
        val image = imageRepository.findImageEntitiesById(id)
    ...
    override fun findAllVideo(id: String): MutableList<FileModel>? {
        val videos = videoRepository.findAllVideoModelByUsersEmail(id)
        return if (videos.isNotEmpty()) {
            val videoResponse = mutableListOf<FileModel>()
            for (video in videos) {
                videoResponse.add(FileModel(id = video.id, name = video.name, type = video.type, data = video.data))
            }
            videoResponse
        } else {
            mutableListOf()
    ...
    override fun uploadAllVideo(email: String, files: MutableList<FileModel>): Boolean {
        val user = userDao.findUserByEmail(email)
        return run {
            for (video in files) {
                videoRepository.save(VideoEntity(name = video.name, type = video.type, data = video.data, users = user))
            }
            true
        }
    }

    override fun removeVideo(id: Long): Boolean {
        val video = videoRepository.findVideoEntitiesById(id)
